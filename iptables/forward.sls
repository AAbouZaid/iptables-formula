  ...
  {% set forward = firewall.get('forward', firewall) %}
  {% set icmp = forward.get('icmp', firewall.icmp) %}
  {% set strict_mode = forward.get('strict', firewall.strict) %}
  {% set block_nomatch = forward.get('block_nomatch', firewall.block_nomatch) %}
  ...
  {%- endif %}
      ...
      iptables_forward_allow_localhost:
        iptables.{{ action }}:
          - table: filter
          - chain: FORWARD
          - jump: ACCEPT
  ...
  {%- set whitelist = forward.get('whitelist', firewall.whitelist) %}
  ...
  # Rules for services
  {%- for service_name, service_details in forward.get('services', firewall.services|default({})) %}
